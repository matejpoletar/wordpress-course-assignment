...
export const FeaturedContentOptions = ({ attributes, setAttributes }) => {
		...
		return (
			<>
				<Notice
					...
					isDismissible={false}
				>
					{__('These options can only be used in a single post. Keep in mind that, by selecting this option, the editor preview may not show the correct items!', 'infinum-academy')}
				</Notice>
				<br />
			</>
		);
	...
	return (
		<PanelBody title={__('Featured content', 'infinum-academy')}>
			{postTypeOptions.length > 0 &&
				<>
					<CustomSelect
						label={<IconLabel icon={icons.file} label={__('Filter by content type', 'infinum-academy')} />}
					...
					{taxonomyOption().length > 0 &&
						<CustomSelect
							label={<IconLabel icon={icons.editOptions} label={__('Filter by taxonomy', 'infinum-academy')} />}
					...
					{featuredContentTaxonomy &&
						<>
							<IconToggle
								...
								label={__('Select specific items', 'infinum-academy')}
								help={__('These options will allow you to manually select items to show.', 'infinum-academy')}
							...
							{useSpecificTerms ?
								<CustomSelect
									label={<IconLabel icon={icons.clipboard}  label={__('Show only specific items', 'infinum-academy')} />}
									help={__('You will preview the latest 30 items, use the search option to find the item you are looking for. If nothing is selected, all items are shown.', 'infinum-academy')}
								...
								<>
									<IconToggle
										...
										label={__('Use term of the current post', 'infinum-academy')}
										help={__('This option allows you to show items in the current post taxonomy. In order for this to work you must select value in the Filter by taxonomy option.', 'infinum-academy')}
					...
					{!featuredContentTaxonomy &&
						<>
							<IconToggle
								icon={icons.visible}
								label={__('Select specific items to show?', 'infinum-academy')}
								checked={useSpecificPosts}
								help={__('This option allows you to select individual items to show.', 'infinum-academy')}
								onChange={() => {
							...
							{useSpecificPosts &&
								<CustomSelect
									label={<IconLabel icon={<BlockIcon iconName='esf-select' />}  label={__('Show only specific items', 'infinum-academy')} />}
									help={__('You will preview the latest 30 items, use the search option to find the item you are looking for. If nothing is selected, all items are shown.', 'infinum-academy')}
			...
			<IconToggle
				icon={icons.none}
				label={__('Exclude the current post', 'infinum-academy')}
				checked={featuredContentExcludeCurrentPost}
				help={__('Exclude the current post from the featured content block items.', 'infinum-academy')}
			...
			<IconToggle
				...
				label={__('Use random order', 'infinum-academy')}
				checked={featuredContentRandomOrder}
				help={__('Show posts in random order.', 'infinum-academy')}
