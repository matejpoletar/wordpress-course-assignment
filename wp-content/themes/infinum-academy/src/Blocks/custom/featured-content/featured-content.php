...
use InfinumAcademy\Filters\BooksFilter;
...
if ($featuredContentPostType === 'books-api') {
	$items = apply_filters(BooksFilter::BOOKS_FILTER, []);
} else {
	...
	if (!$featuredContentServerSideRender) {
		echo Components::render(
			...
			Components::props('loadMore', $attributes, [
				'loadMoreInitiaItems' => wp_json_encode($items),
