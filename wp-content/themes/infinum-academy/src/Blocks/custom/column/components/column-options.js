...
export const ColumnOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Column', 'infinum-academy')}>

			<HelpModal
				type='column'
				buttonLabel={__('How to use a Column?', 'infinum-academy')}
				modalLabel={__('Column', 'infinum-academy')}
			/>
			...
			<Responsive label={<IconLabel icon={icons.order} label={__('Order', 'infinum-academy')} />}>
				{breakpoints.map((keyName, index) => {
					...
					return (
						<Fragment key={keyName}>
							...
							/>
							{index === 0 &&
								<p className='es-decorative-text'>{__('Needs to be set on all columns!', 'infinum-academy')}</p>
							}
						</Fragment>
					);
				})}
			...
			<Responsive label={<IconLabel icon={icons.verticalAlign} label={__('Vertical align', 'infinum-academy')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
			...
			<Responsive label={<IconLabel icon={icons.horizontalAlign} label={__('Horizontal align', 'infinum-academy')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
