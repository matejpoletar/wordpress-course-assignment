...
export const ColumnsOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Columns', 'infinum-academy')}>
			<HelpModal
				type="columns"
				buttonLabel={__('How to use Columns?', 'infinum-academy')}
				modalLabel={__('Columns', 'infinum-academy')}
			/>

			<br /><br />

			<Responsive label={<IconLabel icon={icons.gutter} label={__('Column spacing', 'infinum-academy')} />}>
			...
			<Responsive label={<IconLabel icon={icons.verticalSpacing} label={__('Row spacing', 'infinum-academy')} />}>
				{breakpoints.map((keyName) => {
					const point = ucfirst(keyName);
