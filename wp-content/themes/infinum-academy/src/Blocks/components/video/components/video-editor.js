		...
		<>
			{!hasVideo && showVideoUrl &&
				<MediaPlaceholder
					...
					labels={{
						title: __('Video', 'infinum-academy'),
						instructions: __('Upload a video file or pick one from your media library. You can select multiple video files to have fallbacks with different video formats.', 'infinum-academy'),
					}}
					multiple
					accept={videoAccept}
			...
			{!hasVideo && !showVideoUrl && showPlaceholder &&
				<Placeholder icon={icons.video} label={__('Add a video', 'infinum-academy')}>
					{__('Check the block options in the sidebar', 'infinum-academy')}
				</Placeholder>
			}
