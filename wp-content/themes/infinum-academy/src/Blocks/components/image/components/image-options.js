	...
	return (
		<>
			...
			{imageUse &&
				<>
					{showImageUrl &&
						<Responsive label={<IconLabel icon={icons.image} label={__('Image source', 'infinum-academy')} />}>
							{Object.keys(checkAttrResponsive('imageUrl', attributes, manifest)).map(function (keyName) {
								...
								if (point === 'Large') {
									point = '';
									pointLabel = __('Default (all screens)', 'infinum-academy');
								}
					...
					{showImageAlt &&
						<TextareaControl
							label={<IconLabel icon={icons.altText} label={__('Alt text', 'infinum-academy')} />}
							value={imageAlt}
					...
					{showImageFull &&
						<IconToggle
							icon={icons.size}
							label={__('Fill container', 'infinum-academy')}
							help={__('Span the full width of the container and ignore its maximum width', 'infinum-academy')}
							checked={imageFull}
