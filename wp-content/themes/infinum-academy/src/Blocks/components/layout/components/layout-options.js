...
export const LayoutOptions = (attributes) => {
	...
	return (
		<CollapsableComponentUseToggle
			label={sprintf(__('%s', 'infinum-academy'), label)}
			checked={layoutUse}
		...
		>
			<>
				{showLayoutType &&
					<SimpleVerticalSingleSelect
						label={<IconLabel icon={icons.backgroundTypeAlt} label={__('Layout Type', 'infinum-academy')} />}
				...
				{showLayoutTotalItems &&
					<RangeControl
						label={<IconLabel icon={icons.totalItems} label={__('Maximum number of items to show', 'infinum-academy')} />}
